...
from odoo.addons.ups_shipping_odoo_integration.ups_api.utils import dict2xml, smart_encode
from odoo.addons.ups_shipping_odoo_integration.ups_api.ups_response import Response
from odoo.addons.ups_shipping_odoo_integration.ups_api.exception import ConnectionError
...
class UPS_API():
    def __init__(self, prod_environment, service_name, ups_user_id, ups_password,
                 ups_access_license_number, method="POST", **kwargs):
    ...
    def build_request_data(self, verb, data, version):
        ...
        if not version:
            ...
            xml += "<req:{verb} xmlns:req=\"https://wwwcie.ups.com \" schemaVersion=\"{schema}\">".format(
                    verb=self.verb, schema=version)

        if verb in ["VoidShipmentRequest", "AddressValidationRequest"]:
            base_data = " <?xml version=\"1.0\" encoding=\"UTF-8\"?> <AccessRequest " \
                        "xml:lang=\"en-US\"><AccessLicenseNumber>%s</AccessLicenseNumber><UserId" \
                        ">%s</UserId><Password>%s</Password></AccessRequest><?xml version=\"1.0\" encoding=\"UTF-8\"?>" % (
                self.access_license_number, self.site_id, self.password)
            base_data += data
    ...
    def build_request(self, verb, data, version):
        ...
        request_data = self.build_request_data(verb, data, version)
        request = Request(self.method, self.url, data=request_data, headers=headers)
    ...
    def execute_request(self):
        ...
        self.response = self.session.send(self.request, verify=True, proxies=self.proxies,
    ...
    def error(self):
        ...
        if len(error_array) > 0:
            error_string = "{verb}: {message}".format(verb=self.verb,
                                                      message=", ".join(error_array))
            return error_string
        return None
