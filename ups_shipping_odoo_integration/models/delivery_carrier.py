                    ...
                    if picking.sale_id:
                        ...
                        if mrp_module and mrp_module.state == 'installed':
                            if rec.move_id.bom_line_id and rec.move_id.bom_line_id.bom_id:
                                ...
                                parcel_value_for_bulk_weight += find_sale_line_id.price_unit * rec.quantity + find_sale_line_id.price_unit * rec.quantity * ((find_sale_line_id.tax_ids and sum(find_sale_line_id.tax_ids.mapped('amount')) or 0.0)/100)
                        else:
                            parcel_value_for_bulk_weight += find_sale_line_id.price_unit * rec.quantity + find_sale_line_id.price_unit * rec.quantity * ((find_sale_line_id.tax_ids and sum(find_sale_line_id.tax_ids.mapped('amount')) or 0.0)/100)
                    else:
