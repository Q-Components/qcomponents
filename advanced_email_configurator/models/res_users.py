...
class ResUser(models.Model):
    ...
    def _compute_required_field(self):
        for record in self:
            is_auto_generate_mail_server = self.env['ir.config_parameter'].sudo().get_param(
                'tko_mail_smtp_per_user.is_auto_generate_mail_server')
            if is_auto_generate_mail_server:
    ...
    def test_connection(self):
        ...
        for server in current_user:
            ...
            try:
                ...
                if not email_from:
                    raise UserError(_('Please configure an email on the current user to simulate sending an email message via this outgoing server'))

                # Testing the MAIL FROM step should detect sender filter problems
                (code, repl) = smtp.mail(email_from)

                if code != 250:
                    raise UserError(_('The server refused the sender address (%(email_from)s) with error %(repl)s') % locals())

                # Testing the RCPT TO step should detect most relaying problems
                (code, repl) = smtp.rcpt(email_to)

                if code not in (250, 251):
                    raise UserError(_('The server refused the test recipient (%(email_to)s) with error %(repl)s') % locals())
                ...
                if code != 354:
                    raise UserError(_('The server refused the test connection with error %(repl)s') % locals())
                
            except UserError as e:
                raise e # let UserErrors (messages) bubble up
                ...
                try:
                    ...
                    pass    # ignored, just a consequence of the previous exception

            #exit-main-finally-block
            ...
            self._cr.commit()
            _logger.info(_("Connection Test Succeeded! Everything seems properly set up! for user %s" % self.email))
